  ...
  useEffect(() => {
    ...
    async function pollStatus() {
      try {
        ...
        console.log("BUNNY STATUS:", data);

        /*
          PROGRESS
        */

        const currentProgress = Number(data.encodeProgress || 0);
      ...
      <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            {ready ? "Video processing completed" : "Encoding your video"}
          </p>
        </div>
      ...
      <div className="space-y-6 p-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium text-white">
              {ready ? "Video Ready" : "Processing Video"}
            </span>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-white/10 bg-black/40 p-5">
            ...
            <p className="mt-2 font-medium text-white">{duration}</p>
          </div>
        </div>
      </div>
    </div>
